  ...
  create or replace   view NBA_MONTE_CARLO.PUBLIC.nfl_reg_season_actuals
  
   as (
    with
    cte_wins as (
        select winning_team, count(*) as wins
        from NBA_MONTE_CARLO.PUBLIC.nfl_latest_results
        group by all
    ),
...
from NBA_MONTE_CARLO.PUBLIC.nfl_teams t
